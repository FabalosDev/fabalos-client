<script lang="ts">
  import '../styles/global.css'; // Verified path based on your structure
  import '../styles/app.css';    // Verified path based on your structure
  import { page } from '$app/stores';

  export let data;

  // 🛡️ Reactive check: safely grab the user from the session
...
<svelte:head>
  ...
  <meta property="og:title" content="Fabalos Automation — Intelligent Systems Architecture" />
  <meta
...
>
  ...
  {#if user}
    <div
      class="fixed bottom-0 right-0 z-50 rounded border border-white/10 bg-black/80 px-3 py-1 font-mono text-xs tracking-widest text-blue-500/50 uppercase shadow-2xl backdrop-blur-md"
    >
      ID: <strong class="text-blue-400">{user.email}</strong>
    </div>
  {:else}
    <div
      class="fixed bottom-4 right-4 z-50 hidden font-mono text-[10px] tracking-widest text-red-500/50 uppercase opacity-50 md:block"
    >
      // Not_Authenticated
    </div>
  {/if}
