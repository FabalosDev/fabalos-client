  ...
  <div class="w-full max-w-md rounded-lg border border-white/5 bg-[#111] p-8 shadow-2xl">
    <div class="mb-8 flex items-center justify-between border-b border-white/5 pb-4">
      <div class="flex items-center gap-2">
        ...
        <span class="text-[10px] tracking-[0.2em] text-white/50 uppercase">Security_Protocol: OTP</span>
      </div>
    ...
    {#if message}
      <div class="mt-8 rounded border border-white/5 bg-black/50 p-4">
        <p class="text-[10px] {message.includes('ERROR') ? 'text-red-500' : 'text-blue-400'} leading-relaxed italic">
          > {message}
        </p>
      </div>
    {/if}
    ...
    <div class="mt-8 text-center">
      <a href="/" class="text-[10px] tracking-widest text-white/20 uppercase transition-colors hover:text-white/50">
        ← Abort_Operation
      </a>
    </div>
  </div>
